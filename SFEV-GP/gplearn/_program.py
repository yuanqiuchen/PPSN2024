...
from .functions import _sympol_map
...
class _Program(object):
    ...
    def build_program(self, random_state, withDist=False):
        ...
        function = random_state.randint(0, len(self.function_set) - 1)
    ...
    def get_expression(self):
        ...
        output = ''
        stack = []
        X_num = [] #the variable of the formula
        for i, node in enumerate(self.program):
            # print("node: ", node)
            if isinstance(node, _Function):
                terminals.append(node.arity)
    ...
    def get_subtree(self, random_state, program=None):
        ...
        # print("probs: ", probs)
        start = np.searchsorted(probs, random_state.uniform(0, 1))
        # print("start: ", start)
...
class Individual(_Program):
    def __init__(self, function_set, arities, init_depth, init_method, n_features, const_range, metric, p_point_replace, parsimony_coefficient, random_state, transformer=None, feature_names=None, program=None):
        super().__init__(function_set, arities, init_depth, init_method, n_features, const_range, metric, p_point_replace, parsimony_coefficient, random_state, transformer, feature_names, program)