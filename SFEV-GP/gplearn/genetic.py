...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, params):
    ...
    current_best = params['current_best']
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def updateAttrs(self):
        self.population_size = self.space.population_size

    def updateAttrs(self, space):
    ...
    def _verbose_reporter(self, run_details=None):
        ...
        if run_details is None:
            print('    |{:^25}|{:^42}|'.format('Population Average',
                                               'Best Individual'))
            print('-' * 4 + ' ' + '-' * 25 + ' ' + '-' * 42 + ' ' + '-' * 10)
            line_format = '{:>4} {:>4} {:>8} {:>16} {:>8} {:>16} {:>16} {:>10}'
            print(line_format.format('space', 'Gen', 'Length', 'Fitness', 'Length',
            ...
            oob_fitness = 'N/A'
            line_format = '{:4d} {:4d} {:8.2f} {:16g} {:8d} {:16g} {:>16} {:>10}'
            if self.max_samples < 1.0:
                oob_fitness = run_details['best_oob_fitness'][-1]
                line_format = '{:4d} {:4d} {:8.2f} {:16g} {:8d} {:16g} {:16g} {:>10}'

            print(line_format.format(run_details['space_logo'][-1],
    ...
    def fit(self, X, y, runNum, fileNum, top, sample_weight=None):
        ...
        for gen in range(self.prior_generations, self.prior_generations + self.run_num):

            start_time = time()

            params['current_best'] = self.current_best
            ...
            if self.best_program_:
                population.append(self.best_program_)
            ...
            population.sort(key=compare)
            self.sorted_population = population

            self.current_top100 = []
            for i in range(100):
                self.current_top100.append(population[i])
            
            self.current_best = self.current_top100[0]
            print("current_best: ", self.current_best.raw_fitness_)
            
            self.history_top100 += self.current_top100
            self.history_top100.sort(key=compare)
            temp_ = []
            for i in range(100):
                temp_.append(self.history_top100[i])
            self.history_top100 = temp_
            
            self.history_best = self.history_top100[0]

            outPath = r"/home/qc/test2/new/EVGP6/result/"

            if top == None:
                self.top = self.history_top100[0]
            else:
                if top.raw_fitness_ > self.history_top100[0].raw_fitness_:
                    self.top = self.history_top100[0]
                else:
                    self.top = top
            
            top_file = open(outPath + str(runNum) + '_top_evgp_korns_three_' + str(fileNum) + '.txt', 'a', encoding='utf-8')
            top_file.write(str(self.space_logo) + ' ' + str(self.space_visit) + ' ' +str(self.top.raw_fitness_) + ' ' + str(self.top.get_expression()) + ' ' + str(self.top.length_))
            top_file.write('\n')
            top_file.close()
            ...
            self.best_program_ = best_program
            ...
            self.run_details_['space_logo'].append(self.space_logo)
            oob_fitness = np.nan
            if self.max_samples < 1.0:
        ...
        if isinstance(self, TransformerMixin):
            print("isinstance(self, TransformerMixin)")
